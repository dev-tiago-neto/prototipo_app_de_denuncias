...
from datetime import datetime # Para comparar datas e fazer busca de denuncias por filtro de tempo
...
def triarReportPorSigla(filtroDigitado, indexSigla): # Busca denúncias pelo filtro de Tipo de denúncia, adicionando-as em uma lista
    ...
    for i in range(len(allReports)):                    # Varre a lista de todas as denúncias
        if allReports[i][indexSigla] == filtroDigitado: # Se a sigla dessa denúncia for igual à sigla pedida pelo usuário,
            denunciasFiltradas.append(allReports[i])    # Pinça essa denúncia e a adiciona a uma lista de denúncias filtradas
            qnt += 1                                    # Para cada uma pinçada, o contador aumenta, assim teremos o total de denúncias daquela query
    quantidadeFiltradas.append(qnt)                     # Adciona o total contado a uma lista global a ser acessada em outras funções
    return denunciasFiltradas                           # Retorna a lista de denúncias filtradas
...
def menuLerPorTipo():
    ...
    while respTipo == '-':                              # Se input for ' - ', repete a listagem e pede denovo
        listarTiposReports()
        respTipo = respostaUsuario('[-] Voltar | → ')
    respTipo = validarOpcao(codigoSubmenus[8][0], respTipo) # Valida o input até que esteja entre as opções possíveis e transforma em int
    casosFiltroBusca(respTipo, tipoFiltro='tipo')           # Tria todas as denúncias pelo filtro do tipo de denúncia escolhido, e as imprime sequencialmente
...
def menuMinhasDenuncias():                  # Lista as denúncias que o usuário fez
    if meusReports == []:                               # Caso 1: Ainda não fez denúncias
        ...
        for r in meusReports:                           # Caso 2: Denunciou, lista ela(s)
            ...
            print(f'~∿~∿~∿~∿~∿~∿~∿~∿~∿~∿~∿~∿~\n'
                  f'       SAVE ⏇he Oceans\n'
                  f' {tempTipo}, {r[4]}\n',end='')                 # meusReports = [Estado, Cidade, Local, Tipo, Data]
            print(f' {r[2]}\n'.title())
            print(f' {r[0]} - {r[1]}\n'.title())
            print(f'~∿~∿~∿~∿~∿~∿~∿~∿~∿~∿~∿~∿~∿')
            time.sleep(0.1)
