""" !! Colar link github e especificar underline como necessidade """
...
def iterarLerReports(filtroEscolhido, tipoFiltro, indexDoBuscado): #fix
    if tipoFiltro == 'tipo':
        for i in range(len(allReports)):
            if filtroEscolhido == allReports[i][indexDoBuscado]:
                print(f'tipoFiltro = {tipoFiltro}\nallReports[i][i] = {allReports[i][indexDoBuscado]}')     #[DEBUG]
                outTipo = pegaOTipo(int)#?
                # outData =
                # outLocal = 
                # outEstado =
                # outCidade = 
                elemEncontrado = []
                elemEncontrado.extend([outTipo, outData, outLocal, outEstado, outCidade])
...
def casosFiltroBusca(filtroEscolhido, tipoFiltro): #fix
    match tipoFiltro:
        case 'tipo':
            iterarLerReports(filtroEscolhido, tipoFiltro='tipo', indexDoBuscado=3)
        case 'sigla':
            iterarLerReports(filtroEscolhido, ) #fix
        case 'antesDe':
            iterarLerReports()
        case 'depoisDe':
            iterarLerReports()
...
def menuLerReport(): #fix
    print            (f'~∿~∿~∿~∿~∿~∿~∿~∿~∿~∿~∿~∿~∿\n'               # Pede o filtro de busca
                      f'       SAVE ⏇he Oceans       \n'
                      f'                              \n'
...
meusReports = []        # "Minhas Denpuncias", começa vazio
